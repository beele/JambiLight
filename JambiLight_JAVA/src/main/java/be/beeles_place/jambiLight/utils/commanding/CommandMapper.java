...
public class CommandMapper {
    ...
    private void onEventReceived(BaseEvent event) {
                ...
                try {
                    ...
                    if(PersistentCommand.class.isAssignableFrom(commandClass)) {
                        ...
                        if(matchedCommand != null) {
                            ...
                            ((PersistentCommand) command).start(settings, model, event.getPayload());
                            persistentCommands.add((PersistentCommand) command);
                        }

                    } else {
                        command = (ICommand) commandClass.newInstance();
                        command.execute(settings, model, event.getPayload());
                    }
                } catch (Exception e) {
